...
def matchups(deck):
    ...
    scrape = yaha_analyzer.yaha_analyzer()
    scrape.open_collectobot_data()
    data = scrape.generate_decklist_matchups(game_threshold = 3).reset_index()
    ...
    game_count = len(scrape.games['hero'])
    return render_template('matchups.html', ids=ids, graphJSON=graphJSON, game_count = game_count, name=deck)
